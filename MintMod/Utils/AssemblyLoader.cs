            ...
            try {
                ...
                switch (message.StatusCode) {
                    case HttpStatusCode.OK:
                        Con.Msg(ConsoleColor.Green, $"Successfully grabbed {modName}");
                        Task<byte[]> _bytes = message.Content.ReadAsByteArrayAsync();
                        _bytes.Wait();
                        _assenbly = Assembly.Load(_bytes.Result);
                        break;
                    case HttpStatusCode.InternalServerError:
                        Con.Error("The DLL on this server was removed, it is probably getting updated, please try again in 30 seconds.");
                        Con.Error("If you are constantly getting this error, please post in #bug-reports");
                        break;
                    default:
                ...
                Con.Error($"An unknown error occured while attempting to retrieve the {modName} assembly! (Loading fallback)");
                ...
                try {
                    ...
                    switch (Results.StatusCode) {
                        ...
                        case HttpStatusCode.InternalServerError:
                            ...
                            break;
                        default:
                            Con.Error($"An unknown error has occured! HTTP Status Code: {Results.StatusCode}\nUnable to load {modName}!");
                            break;
                    }
                } catch (Exception f) {
                    Con.Error($"First Error:\n{e}");
                    Console.WriteLine("-----------------------------------------------------------------------------------------------");
                    Con.Error($"[FALLBACK] An unknown error occured while attempting to retrieve the {modName} assembly!\n{f}");
                }
            }
